...
def evaluate(opt, save_path='results/phantom'):
    ...
    dataset= MonoDataset(opt.data_path,
                 opt.height,
                 opt.width,
                 4,
                 is_train=False,
    ...
    with torch.no_grad():
        for inputs in dataloader:
            ...
            all_color_aug = torch.cat([inputs[("color", 1, 0)], inputs[("color", 1, 0)]], 1)
            #all_color_aug = inputs["color"]
    ...
    np.savez_compressed(os.path.join(os.path.dirname(__file__), f'{save_path}/predicted_poses.npz'), data=np.array(pred_poses))
    """
